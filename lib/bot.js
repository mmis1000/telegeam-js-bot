...
Or run /[you favorite language]_hello to view hello world example
    ...
    if (message.text != undefined && message.text.match(/^\/[a-z0-9_]+(@[^\s]+)?/)) {
        ...
        if (quotas[message.from.id] >= maxTry) {
            return api.sendMessage(message.chat.id, 'Sorry! You are out of your quota ( ' + maxTry + ' times / ' + maxTry * coolDown / 1000 + ' second ). Please retry later.', 0, additionOptions);
        }
        ...
        p.on('exit', function () {
            ...
            // ensure cleared
            if (outputLength >= outputLimit) {
                api.sendMessage(message.chat.id, '[WARN] some message was ignored because the output is too long.', 0, additionOptions);
            }
