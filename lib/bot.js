...
`, additionOptions).catch(catchHandle);
    ...
    if (message.text != undefined && message.text.match(/^\/[a-z0-9_]+(@[^\s]+)?(\s|\r|\n|$)/)) {
        ...
        if (!languageArr) return api.sendMessage(message.chat.id, 'something went wrong', additionOptions).catch(catchHandle);
        ...
        let text = message.text.replace(/\/[a-z0-9_]+(@[^\s]+)?\s?/, '');
...
use || to terminate the stdin`, additionOptions).catch(catchHandle);
    ...
    runner.on('exit', function (data) {
        clearTimeout(timeoutIdMap.get(runner));
