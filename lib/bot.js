            ...
            try {
                ...
                if (cmd.type === 'stdout') {
                    ...
                    while (stdoutBuffer.length > 1333) {
                        ...
                        api.sendMessage(message.chat.id, part, 0, additionOptions);
                    }
                    
                    clearTimeout(stdoutWaitId);
                    stdoutWaitId = setTimeout(function() {
                        // console.log('output stdout', stdoutBuffer)
                        api.sendMessage(message.chat.id, stdoutBuffer, 0, additionOptions);
                        stdoutBuffer = "";
                    }, 500)
                }
                ...
                if (cmd.type === 'stderr') {
                    ...
                    while (stdoutBuffer.length > 1333) {
                        ...
                        api.sendMessage(message.chat.id, "stderr: " + part, 0, additionOptions);
                    }
                    
                    clearTimeout(stderrWaitId);
                    stderrWaitId = setTimeout(function() {
                        // console.log('output stderr', stdoutBuffer)
                        api.sendMessage(message.chat.id, "stderr: " + stderrBuffer, 0, additionOptions);
                        stderrBuffer = "";
                    }, 500)
                }
                ...
                if (cmd.type === 'exit') {
                    try {
                        ...
                        if ((res.code !== 0 || res.signal != null) && isSilent) {
                            api.sendMessage(message.chat.id, 'program ended with code ' + res.code + ' and signal ' + res.signal, 0, additionOptions);
                        }
                    } catch (e) {
                        console.error(e);
                    }
